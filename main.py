...
def main() -> None:
    ...
    if config.metrics_enabled:
        ...
        if config.host == "":
            # Single dual-stack server: listen on both IPv4 and IPv6
            metrics_server, metrics_thread = start_http_server(addr="::", port=config.metrics_port)
            logger.info(f"Prometheus metrics server started at http://[::]:{config.metrics_port} (dual-stack)")
        else:
